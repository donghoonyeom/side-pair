...
class FeedRepositoryTest {
    ...
    void 단일_피드를_조회한다() {
        ...
        final Member creator = 사용자를_생성한다("test2@email.com", "페어");
        final FeedCategory category = 카테고리를_생성한다("헬스케어");
        final Feed savedFeed = 피드를_저장한다("피드 제목", creator, category);
    ...
    void 카테고리_값이_null이라면_삭제되지_않은_전체_피드를_최신순으로_조회한다() {
        ...
        final Feed gameFeed = 피드를_저장한다("게임 피드", creator, gameCategory);
        final Feed gameFeed2 = 피드를_저장한다("게임 피드2", creator, gameCategory);
        final Feed travelFeed = 피드를_저장한다("이커머스 피드", creator, travelCategory);
        삭제된_피드를_저장한다("이커머스 피드2", creator, travelCategory);
    ...
    void 카테고리_값으로_1이상의_유효한_값이_들어오면_해당_카테고리의_삭제되지_않은_피드를_최신순으로_조회한다() {
        ...
        final Feed gameFeed = 피드를_저장한다("게임 피드", creator, gameCategory);
        final Feed gameFeed2 = 피드를_저장한다("게임 피드2", creator, gameCategory);
        삭제된_피드를_저장한다("게임 피드3", creator, gameCategory);
        삭제된_피드를_저장한다("게임 피드4", creator, travelCategory);
    ...
    void 피드를_제목으로_검색한다() {
        ...
        final Feed feed1 = 피드를_저장한다("피드", creator, category);
        final Feed feed2 = 피드를_저장한다("일피드", creator, category);
        final Feed feed3 = 피드를_저장한다("피 드일", creator, category);
        final Feed feed4 = 피드를_저장한다("일피 드", creator, category);
        피드를_저장한다("피", creator, category);
        삭제된_피드를_저장한다("피드", creator, category);
    ...
    @Test
    void 피드를_크리에이터_닉네임으로_검색한다() {
        // given
        final Member creator1 = 사용자를_생성한다("test@email.com", "페어");
        final Member creator2 = 사용자를_생성한다("test2@email.com", "페어2");
        final FeedCategory category = 카테고리를_생성한다("헬스케어");

        final Feed feed1 = 피드를_저장한다("피드", creator1, category);
        final Feed feed2 = 피드를_저장한다("피드", creator1, category);
        피드를_저장한다("피드", creator2, category);
        final Feed feed4 = 피드를_저장한다("피드", creator1, category);
        피드를_저장한다("피드", creator2, category);
        삭제된_피드를_저장한다("피드", creator1, category);
    ...
    @Test
    void 피드를_태그_이름으로_검색한다() {
        // given
        final Member creator = 사용자를_생성한다("test2@email.com", "페어");
        final FeedCategory category = 카테고리를_생성한다("헬스케어");

        final Feed feed1 = 피드를_태그와_저장한다("피드", creator, category,
                new FeedTags(List.of(
                        new FeedTag(new FeedTagName("자바")),
                        new FeedTag(new FeedTagName("스프링")))));

        피드를_저장한다("피드", creator, category);

        final Feed feed3 = 피드를_태그와_저장한다("피드", creator, category,
                new FeedTags(List.of(
                        new FeedTag(new FeedTagName("자바")))));

        피드를_태그와_저장한다("피드", creator, category, new FeedTags(List.of(
                new FeedTag(new FeedTagName("스프링")))));
    ...
    void 사용자가_생성한_피드를_조회한다() {
        ...
        final Feed gameFeed = 피드를_저장한다("피드1", creator, gameCategory);
        final Feed travelFeed = 피드를_저장한다("피드2", creator, travelCategory);
        final Feed deletedGameFeed = 삭제된_피드를_저장한다("피드3", creator, itCategory);
    ...
    void 사용자_이메일로_피드를_조회한다() {
        ...
        final Feed gameFeed1 = 피드를_저장한다("피드1", creator1, gameCategory);
        final Feed travelFeed = 피드를_저장한다("피드2", creator1, travelCategory);

        final Member creator2 = 사용자를_생성한다("test2@email.com", "페어2");
        피드를_저장한다("피드3", creator2, gameCategory);
    ...
    void 사용자_이메일로_피드를_조회시_없으면_빈_값을_감싸서_반환한다() {
        ...
        피드를_저장한다("피드1", creator1, gameCategory);
        final Feed travelFeed = 피드를_저장한다("피드2", creator1, travelCategory);
    ...
    void 삭제된_피드를_피드_본문과_함께_조회한다() {
        ...
        피드를_저장한다("피드1", creator, gameCategory);
        피드를_저장한다("피드2", creator, travelCategory);
        final Feed deletedFeed = 삭제된_피드를_저장한다("피드", creator, travelCategory);
    ...
    private Feed 피드를_저장한다(final String title, final Member creator, final FeedCategory category) {
        final Feed feed = new Feed(title, "피드 소개글", 10, creator, category);
        feed.addContent(new FeedContent("피드 본문"));
        return feedRepository.save(feed);
    }

    private Feed 삭제된_피드를_저장한다(final String title, final Member creator, final FeedCategory category) {
    ...
    private Feed 피드를_태그와_저장한다(final String title, final Member creator, final FeedCategory category,
    ...
    private Feed 노드_정보를_포함한_피드를_생성한다(final String title, final Member creator, final FeedCategory category) {
        final Feed feed = new Feed(title, "피드 소개글", 10, creator, category);
