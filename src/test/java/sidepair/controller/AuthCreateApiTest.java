...
class AuthCreateApiTest extends ControllerTestHelper {
    ...
    @Test
    void 로그인_시_이메일에_빈값이_들어올_때_예외를_던진다() throws Exception {
        //given
    ...
    void 로그인_시_비밀번호_빈값이_들어올_때_예외를_던진다() throws Exception {
        //given
        final LoginRequest request = new LoginRequest(EMAIL, "");
        final String jsonRequest = objectMapper.writeValueAsString(request);
    ...
    void 로그인_시_이메일와_비밀번호_모두_빈값이_들어올_때_예외를_던진다() throws Exception {
        ...
        assertThat(responses).usingRecursiveComparison()
                ...
                .isEqualTo(List.of(emailResponse, passwordResponse));
    }

    @Test
    void 로그인_시_이메일에_해당하는_회원이_없을_때_예외를_던진다() throws Exception {
        //given
        final LoginRequest request = new LoginRequest(EMAIL, PASSWORD);
        final String jsonRequest = objectMapper.writeValueAsString(request);
        doThrow(new AuthenticationException("존재하지 않는 이메일입니다."))
                .when(authService)
                .login(request);
        ...
        //then
        final ErrorResponse expectedResponse = new ErrorResponse("존재하지 않는 이메일입니다.");
