...
import static sidepair.integration.fixture.AuthenticationAPIFixture.응답을_반환하는_로그인;
import static sidepair.integration.fixture.AuthenticationAPIFixture.토큰_재발행;
...
class AuthenticationIntegrationTest extends InitIntegrationTest {
    ...
    void 정상적으로_로그인에_성공한다() {
        //given
        final LoginRequest 로그인_요청 = new LoginRequest(DEFAULT_EMAIL, DEFAULT_PASSWORD);
    ...
    void 회원이_존재하지_않을때_로그인에_실패한다() {
        //given
        final LoginRequest 로그인_요청 = new LoginRequest("sssss@email.com", "password1!");
        ...
        assertThat(실패_응답_바디.message()).isEqualTo("존재하지 않는 이메일입니다.");
    }

    @Test
    void 비밀번호가_틀렸을때_로그인에_실패한다() {
        //given
        final LoginRequest 로그인_요청 = new LoginRequest(DEFAULT_EMAIL, "password2@");
    ...
    @Test
    void 이메일과_비밀번호에_빈값이_있을때() throws JsonProcessingException {
        //given
